...
// User Management (Collaborator accounts) is Admin-only: a Collaborator has
// read-only access to the rest of the system and must not be able to create
// more accounts, including other Collaborators.
requirePageAdmin();
?>
<!DOCTYPE html>
<html lang="en">
...
<body class="bg-gray-50 dark:bg-gray-900 transition-colors">
            ...
            <main class="flex-1 overflow-y-auto px-6 pt-24 bg-gray-50 dark:bg-gray-900/50 custom-scrollbar">
                <div class="flex justify-between items-center mb-2">
                    ...
                    <button class="open-modal bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-lg font-medium"
                        data-modal="add-Collaborator-modal">
                        Add Collaborator
                    </button>
                </div>
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">Grant a Collaborator no access, read-only, or read+write on each module below. Only an Admin can add or manage a Collaborator.</p>

                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-hidden border border-gray-100 dark:border-gray-700">
                    <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                        <h2 class="text-lg font-semibold text-gray-800 dark:text-white">Collaborators</h2>
                    </div>
                        ...
                        <table id="CollaboratorsTable" class="min-w-full">
                            <thead class="bg-indigo-200 dark:bg-indigo-600">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-700 dark:text-gray-200 uppercase tracking-wider">Access</th>
    ...
    <!-- Add & Edit Modal -->
    <div id="Collaborator-modal" class="modal hidden fixed inset-0 z-50 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-11/12 max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-xl font-bold dark:text-gray-50 text-gray-950" id="modal-title">Add Collaborator</h3>
            ...
            <form id="Collaborator-form">
                ...
                <div class="mb-2">
                    <label class="block text-sm font-medium mb-2 text-gray-900 dark:text-gray-100">Module Access</label>
                    <div class="border rounded-lg divide-y dark:divide-gray-700 dark:border-gray-600">
                        <div class="grid grid-cols-[1fr_auto_auto_auto] gap-2 px-3 py-2 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">
                            <span>Module</span>
                            <span class="w-14 text-center">None</span>
                            <span class="w-14 text-center">Read</span>
                            <span class="w-14 text-center">Write</span>
                        </div>
                        <?php foreach (collaboratorModules() as $moduleKey => $moduleLabel): ?>
                            <div class="grid grid-cols-[1fr_auto_auto_auto] gap-2 items-center px-3 py-2">
                                <span class="text-sm text-gray-800 dark:text-gray-100"><?= htmlspecialchars($moduleLabel) ?></span>
                                <span class="w-14 text-center">
                                    <input type="radio" name="permissions[<?= $moduleKey ?>]" value="none" checked>
                                </span>
                                <span class="w-14 text-center">
                                    <input type="radio" name="permissions[<?= $moduleKey ?>]" value="read">
                                </span>
                                <span class="w-14 text-center">
                                    <input type="radio" name="permissions[<?= $moduleKey ?>]" value="write">
                                </span>
                            </div>
                        <?php endforeach; ?>
                    </div>
    ...
    <script>
        ...
        async function loadCollaborators() {
            try {
                const res = await fetch('controller/user.php?action=get_collaborators');
        ...
        function setPermissionRadios(permissions) {
            Object.keys(MODULE_LABELS).forEach(mod => {
                const level = (permissions && permissions[mod]) || 'none';
                const radio = document.querySelector(`input[name="permissions[${mod}]"][value="${level}"]`);
        ...
        document.addEventListener('DOMContentLoaded', () => {
            document.querySelector('.open-modal').onclick = () => {
                document.getElementById('modal-title').textContent = 'Add Collaborator';
            ...
            document.addEventListener('click', e => {
                const editBtn = e.target.closest('.edit-Collaborator');
                if (editBtn) {
                    document.getElementById('modal-title').textContent = 'Edit Collaborator';
                    ...
                    setPermissionRadios(JSON.parse(editBtn.dataset.permissions || '{}'));
                    document.getElementById('Collaborator-modal').classList.remove('hidden');
                }
            });

            document.getElementById('Collaborator-form').onsubmit = async e => {
                e.preventDefault();
            ...
            document.addEventListener('click', async e => {
                const delBtn = e.target.closest('.delete-Collaborator');
                if (delBtn && confirm('Delete this Collaborator permanently?')) {
                    const res = await fetch('controller/user.php', {
                        method: 'POST',
                    ...
                    const json = await res.json();
                    showToast(json.success ? 'success' : 'error', json.message);
                    if (json.success) loadCollaborators();
                }
            });
