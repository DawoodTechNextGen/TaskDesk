...
<script>
    ...
    const table = $('#interneesTable').DataTable({
        ...
        pageLength: 10,
        language: {
            emptyTable: "No internees found",
            info: "Showing _START_ to _END_ of _TOTAL_ internees",
            infoEmpty: "Showing 0 to 0 of 0 internees",
            infoFiltered: "(filtered from _MAX_ total internees)",
            loadingRecords: "Loading...",
            zeroRecords: "No matching internees found"
        }
    });
                ...
                json.data.forEach(u => {
                    table.row.add([
                        ...
                        `<div class="flex items-center space-x-2">
                            ...
                            ${(u.completion_rate >= 75 && u.months_completed >= 3) ? `
                            <button class="approve-certificate ${(u.approve_status !== 1) ? 'text-amber-300 hover:text-amber-400' : 'text-green-500 hover:text-green-600'} transition-colors" 
                                    data-id="${u.id}"
                                    data-approved="${u.approve_status === 1}"
                                    title="${u.approve_status === 1 ? 'Certificate Approved' : 'Approve Certificate'}">
                                <svg width="20px" height="20px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Delete Internee with loading state
        document.addEventListener('click', async e => {
            const delBtn = e.target.closest('.delete-internee');
            if (delBtn) {
                if (!confirm('Are you sure you want to delete this internee? This action cannot be undone.')) {
                    return;
                }
                
                const originalHTML = delBtn.innerHTML;
                delBtn.innerHTML = '<span class="delete-loader"></span>';
        ...
        document.addEventListener('click', async function(e) {
            ...
            if (approveBtn && approveBtn.dataset.approved !== "true") {
                const internId = approveBtn.dataset.id;
                const internName = approveBtn.closest('tr').querySelector('td:nth-child(2)').textContent;
                
                if (!confirm(`Approve certificate for ${internName}?`)) {
                    return;
                }
                
                const originalHTML = approveBtn.innerHTML;
                approveBtn.innerHTML = '<span class="approve-loader"></span>';
