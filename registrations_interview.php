...
<style>
    ...
    /* Time indicators */
    .time-soon { color: #dc2626; font-weight: 600; }
    .time-today { color: #059669; font-weight: 600; }
    ...
    /* Calendar styling */
    #calendarDays button { min-height: 2.5rem; }
    #calendarDays button:hover:not(:disabled) { transform: scale(1.05); transition: transform 0.2s ease; }
    #calendarDays button:disabled { cursor: not-allowed; }
    
    /* Modal scrollbar styling */
    #rescheduleModal div[class*="max-h-"]::-webkit-scrollbar { width: 6px; }
    #rescheduleModal div[class*="max-h-"]::-webkit-scrollbar-track { background: #f1f1f1; border-radius: 3px; }
    #rescheduleModal div[class*="max-h-"]::-webkit-scrollbar-thumb { background: #888; border-radius: 3px; }
    #rescheduleModal div[class*="max-h-"]::-webkit-scrollbar-thumb:hover { background: #555; }
    .dark #rescheduleModal div[class*="max-h-"]::-webkit-scrollbar-track { background: #374151; }
    .dark #rescheduleModal div[class*="max-h-"]::-webkit-scrollbar-thumb { background: #6b7280; }
    .dark #rescheduleModal div[class*="max-h-"]::-webkit-scrollbar-thumb:hover { background: #9ca3af; }
    
    /* Conflict highlighting */
    .time-slot-conflict { border-color: #f87171 !important; background-color: #fef2f2 !important; color: #dc2626 !important; }
    .time-slot-conflict:focus { border-color: #dc2626 !important; ring-color: #fca5a5 !important; }
    @keyframes fadeIn { from { opacity: 0; transform: translateY(-10px); } to { opacity: 1; transform: translateY(0); } }
    #timeSlotAlert > div { animation: fadeIn 0.3s ease-out; }
</style>
...
<body class="bg-gray-50 dark:bg-gray-900 transition-colors">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <div class="flex items-center space-x-2">
                            <button class="filter-btn bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300" data-filter="all">All</button>
                            <button class="filter-btn bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300" data-filter="today">Today</button>
                            <button class="filter-btn bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300" data-filter="upcoming">Upcoming</button>
                            <button class="filter-btn bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300" data-filter="past">Past</button>
                        </div>
                        
                        <!-- Date Range Filter -->
                        <div class="flex items-center space-x-2">
                            <input type="date" id="dateFrom" class="px-3 py-1.5 border rounded text-sm dark:bg-gray-700 dark:border-gray-600">
                            <span class="text-gray-500 dark:text-gray-400">to</span>
                            <input type="date" id="dateTo" class="px-3 py-1.5 border rounded text-sm dark:bg-gray-700 dark:border-gray-600">
                            <button id="applyDateFilter" class="px-3 py-1.5 bg-blue-600 text-white rounded text-sm hover:bg-blue-700">Apply</button>
                            <button id="clearDateFilter" class="px-3 py-1.5 bg-gray-300 dark:bg-gray-600 text-gray-700 dark:text-gray-300 rounded text-sm">Clear</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                    <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow border border-gray-100 dark:border-gray-700">
                        <div class="flex items-center">
                            ...
                            <div>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Total Interviews</p>
                                <h3 id="totalInterviews" class="text-2xl font-bold text-gray-800 dark:text-white">0</h3>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow border border-gray-100 dark:border-gray-700">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <h3 id="todayInterviews" class="text-2xl font-bold text-gray-800 dark:text-white">0</h3>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow border border-gray-100 dark:border-gray-700">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <h3 id="upcomingInterviews" class="text-2xl font-bold text-gray-800 dark:text-white">0</h3>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow border border-gray-100 dark:border-gray-700">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <h3 id="noshowInterviews" class="text-2xl font-bold text-gray-800 dark:text-white">0</h3>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                            ...
                            <textarea id="viewNotes" class="w-full px-3 py-2 border rounded text-sm dark:bg-gray-800 dark:border-gray-600" rows="3" placeholder="Add interview notes..."></textarea>
                        ...
                        <div class="flex justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <button type="button" id="viewRejectBtn" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">
                                Reject
                            </button>
                        </div>
                    </div>
                        ...
                        <div id="step3" class="space-y-6 hidden">
                            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Reschedule Reason <span class="text-red-500">*</span></label>
                                        <textarea id="reschReason" name="reason" class="w-full px-3 py-2.5 border rounded-lg dark:bg-gray-800 dark:border-gray-600" rows="2" placeholder="Enter reason for rescheduling..." required></textarea>
                                    </div>
                                </div>
                    ...
                    <form id="hireForm" class="space-y-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" id="hireInterviewDate" class="w-full px-3 py-2.5 border rounded-lg text-gray-800 dark:text-gray-200" readonly>
                            </div>
            ...
            <div id="rejectModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
                <div class="bg-white dark:bg-gray-800 rounded-xl p-6 w-full max-w-md">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-xl font-bold text-gray-800 dark:text-white">Reject Candidate</h3>
                        <button type="button" id="closeRejectModal" class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                            </svg>
                        </button>
                    ...
                    <form id="rejectForm" class="space-y-6">
                        <input type="hidden" id="rejectId" name="id">
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Candidate Name</label>
                            <input type="text" id="rejectName" class="w-full px-3 py-2.5 border rounded-lg text-gray-800 dark:text-gray-200" readonly>
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Rejection Reason</label>
                            <select id="rejectReason" name="reason" class="w-full px-3 py-2.5 border rounded-lg dark:bg-gray-800 dark:border-gray-600" required>
                                <option value="">Select Reason</option>
                                <option value="Not Qualified">Not Qualified</option>
                                <option value="No Show">No Show</option>
                                <option value="Not Interested">Not Interested</option>
                                <option value="Technical Skills Mismatch">Technical Skills Mismatch</option>
                        ...
                        <div>
                            ...
                            <textarea name="notes" class="w-full px-3 py-2.5 border rounded-lg dark:bg-gray-800 dark:border-gray-600" rows="3" placeholder="Add any additional notes..."></textarea>
                        </div>

                        <div class="flex justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                            <button type="button" id="cancelRejectBtn" class="px-4 py-2.5 border rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">
                                Cancel
                            </button>
                            <button type="submit" class="px-4 py-2.5 bg-red-600 text-white rounded-lg hover:bg-red-700">
                                Reject Candidate
                            </button>
                        </div>
                    </form>
    ...
    function updateSummaryCards(data) {
        document.getElementById('totalInterviews').textContent = data.total || 0;
        document.getElementById('todayInterviews').textContent = data.today || 0;
        document.getElementById('upcomingInterviews').textContent = data.upcoming || 0;
        document.getElementById('noshowInterviews').textContent = data.noshow || 0;
    }
    ...
    const expandableColumns = ['email', 'cnic', 'city', 'country', 'platform', 'interview_notes', 'scheduled_by_name', 'scheduled_at'];
    const headerMap = { 
        id: 'ID', 
        name: 'Name', 
        email: 'Email', 
    ...
    function formatDetails(row) {
        ...
        return `<div class="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg grid grid-cols-2 gap-4 text-sm">${details}</div>`;
    }
    ...
    function updateProgress() {
        ...
        steps.forEach((id, index) => {
            ...
            const $wrapper = $(`#${id}`); // This selector logic is a bit loose, let's fix it based on HTML structure
    ...
    function updateConfirmationDetails() {
        ...
        const dateStr = selectedDate.toLocaleDateString('en-US', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' });
    ...
    function initCalendar() { renderCalendar(currentDate); }
    ...
    async function selectDate(date) {
        ...
        updateTimeSlotDisplay(date.toISOString().split('T')[0]);
        validateTimeInput();
    }

    function setDefaultTimes() {
        // Set default to 09:00 - 10:00
        $('#intFromTime').val('00:00');
        $('#intToTime').val('00:00');
    ...
    async function fetchBookedSlots(date) {
        ...
        } catch(e) { return {success:false, slots:[]}; }
    }
    ...
    async function checkTimeSlotConflict(date, start, end, id) {
        try {
            ...
            return { hasConflict: !json.success && json.type === 'conflict', message: json.message };
        } catch(e) { return { hasConflict: false }; }
    }

    function showAlert(title, msg) {
        alert(`${title}\n${msg}`); // Simple alert for now or implement custom modal if needed
    }
    ...
    $(document).ready(function() {
        ...
        const table = $('#interviewTable').DataTable({
            ...
            ajax: {
                ...
                data: function(d) {
                    ...
                    if (dateFilter.from) d.date_from = dateFilter.from;
                    if (dateFilter.to) d.date_to = dateFilter.to;
                },
            ...
            columns: [
                ...
                { 
                    ...
                    render: function(data, type, row) {
                        ...
                        const formattedTime = row.interview_start ? formatDateTime(row.interview_start).time : 'Not scheduled';
                        ...
                        return `
                            <div>
                                ...
                                <div class="text-xs ${timeClass}">${formattedTime}</div>
                ...
                { 
                    data: 'interview_start',
                    render: function(data, type, row) {
                        if (!data) return '<span class="text-gray-400 italic">Not scheduled</span>';
                        
                        const dt = formatDateTime(data);
                        const timeStatus = getTimeStatus(data);
                        let rowClass = '';
                        
                        if (timeStatus === 'today') rowClass = 'upcoming-interview';
                        
                        return `
                            <div class="${rowClass} p-2">
                                <div class="font-medium">${dt.time}</div>
                                <div class="text-xs text-gray-500">${dt.date}</div>
                                ${row.interview_end ? `<div class="text-xs text-gray-400">Ends: ${formatDateTime(row.interview_end).time}</div>` : ''}
                            </div>
                        `;
                    }
                },
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        <div class="flex flex-wrap gap-1">
                            ...
                            <button class="action-btn bg-red-500 hover:bg-red-600 text-white reject-btn" 
            ...
            order: [[5, 'asc']], // Sort by interview time
            language: { 
        ...
        function loadViewModal(row) {
            ...
            $('#viewNotes').val(row.interview_notes || '');
            
            // Store current row data
            $('#viewModal').data('row', row);
            
            // Set button actions
            $('#viewRescheduleBtn').off('click').on('click', function() {
                $('#viewModal').addClass('hidden');
                openRescheduleModal(row);
            });
            
            $('#viewHireBtn').off('click').on('click', function() {
                $('#viewModal').addClass('hidden');
                openHireModal(row);
            });
            
            $('#viewRejectBtn').off('click').on('click', function() {
                $('#viewModal').addClass('hidden');
                openRejectModal(row);
            });
        ...
        $('#intPlatform').on('change', function() {
            if(this.value) { $('#platformError').addClass('hidden'); $(this).removeClass('border-red-500'); }
        });
        ...
        $('#rescheduleForm').on('submit', async function(e) {
            ...
            if (!$('#reschReason').val()) { showToast('error', 'Please enter a reason'); return; }
            ...
            try {
                ...
                formDataObj.append('interview_start', `${formData.date.toISOString().split('T')[0]} ${formData.startTime}:00`);
                formDataObj.append('interview_end', `${formData.date.toISOString().split('T')[0]} ${formData.endTime}:00`);
                formDataObj.append('reason', $('#reschReason').val());
                
                const res = await fetch('controller/registrations.php', { method: 'POST', body: formDataObj });
                const json = await res.json();
        ...
        function openHireModal(row) {
            ...
            $('#hireInterviewDate').text(formatDateTime(row.interview_start).full);
        ...
        $('#closeRejectModal, #cancelRejectBtn').on('click', () => $('#rejectModal').addClass('hidden'));
        ...
        // Initialize today as default in date inputs
        const today = new Date().toISOString().split('T')[0];
        $('#dateFrom').val(today);
        $('#dateTo').val(today);

        // Set "Today" as active filter by default
        $('.filter-btn[data-filter="today"]').addClass('active');
        currentFilter = 'today';
        table.ajax.reload();
    });
</script>
